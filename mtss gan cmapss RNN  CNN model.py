...
condition_vector = condition_vector[:,sequence_length-1,:]
del all_data, all_label
#train_data, train_label,max_value_rul,max_values = dp.preprocessing_cmapps(sl=seq_length,path ='Data/train_FD001.txt',rul_path = 'Data/RUL_FD001.txt', short=True)
Z_dim = 50 #size of latent vector (noise vector)
#gen_concat = np.concatenate([train_data[:,:,3:4],train_label],axis=2)
features_n = train_data.shape[2]+1 #RUL column will be later added
...
def train(models, data, params):
    ...
    for i in range(train_steps):
        ...
        print('real_labels: ' + str(real_labels.shape))
