...
die cycle und setting spalten bei cmapss fliegen raus, es wird nur RUL an die Zeitreihen angehängt
'''

#Label for Discriminator real = 1 and fake = 0
seq_length = sequence_length = 24 #specifying the lenght of series
patience_encoder = 70
encoder_epochs = 10
flat_rul = True
dataset_n ='FD001'
...
condition_vector = np.concatenate([train_label,all_clusters[0:train_label.shape[0],:]], axis = 2) #only the cluster information for the train_label
#lass nur die letzte Zeile nehmen und den Discriminator diese als zusätzlichen input füttern.
#der generator bekommt auch nur eine Zeile, muss die Daten erzeugen, aber ohne RUL Spalte
condition_vector = condition_vector[:,sequence_length-1,:] #nehme nur die letzte Zeile oder doch die gesamte Zeitreihensequenz??

del all_data, all_label
Z_dim = 50 #size of latent vector (noise vector)
...
cond_length = condition_vector.shape[1]

num_labels = cond_length
noise_dim = seq_length*features_n
SHAPE = (train_data.shape[1], train_data.shape[2]) #Shape der Daten muss noch unterschei

intermidiate_dim = int((train_data.shape[1]*train_data.shape[2]-cond_length)/2)
...
def generator(inputs,
    ...
    x = Dense(SHAPE[0]*SHAPE[1])(x) 
    x = Reshape((SHAPE[0], SHAPE[1]))(x)
    x = Conv1D(filters=18, kernel_size=2, strides=1, padding='same')(x)
    ...
    x = LeakyReLU(alpha=0.1)(x)
    x = Dropout(0.1)(x)
    x = Dense(units=features_n)(x)
    x = Reshape((seq_length,features_n))(x)
    x = BatchNormalization(momentum=0.8)(x)
    '''
...
#feature dim zu intermediate dim bzw- auch der output shape von gen0 (auch ein )
def build_encoder(inputs, num_labels):#int(features_n*seq_length/2)): #num_labels=features_n
    ...
    y = Dense(units=intermidiate_dim+int(intermidiate_dim/2))(y)
    y = Dense(units=intermidiate_dim, activation='relu')(y)
    # Encoder0 or enc0: data to feature0 
    enc0 = Model(inputs=x, outputs=y, name="encoder0")
    
    # Encoder1 or enc1
    print('num_labels: ' + str(num_labels))
    y = Dense(units = intermidiate_dim)(feature0)
    y = Dense(units=num_labels)(y) 
    labels = Activation('relu')(y)
    # Encoder1 or enc1: feature0 to class labels 
    #enc1 = Model(inputs=feature0, outputs=labels, name="encoder1") #feature0
    enc1 = Model(inputs=feature0, outputs=labels, name="encoder1") #feature0
    # return both enc0 and enc1
    return enc0, enc1

def build_generator(latent_codes):#SHAPE[0]*SHAPE[1]):
    ...
    x = Dense(128, activation='relu')(x)
    x = BatchNormalization()(x)
    fake_feature0 = Dense(intermidiate_dim, activation='relu')(x)
...
def build_and_train_models(train_steps = 2000):
    ...
    train_n = int((train_data.shape[0])*.98)
    #X = data[:,:,:-1]
    #y = data[:,-1,-1]
    ...
    label = np.concatenate([cycle,col7,train_label],axis=2) #col7 = settings for cmapss
    del clusters
    ...
    z_shape = (z_dim, )
    feature0_dim = intermidiate_dim#int(SHAPE[0]*SHAPE[1]/2)
    ...
    #x are time series sequences, y are condition  + cluster as 1d flatten vector
    ...
    gen0, gen1,dis0,dis1, losses,acc_fake,acc_real, adv0, adv1  = train(models, data, params)


    return gen0, gen1, dis0, dis1, losses, acc_fake,acc_real, adv0, adv1
...
def train(models, data, params):
    ...
    for i in range(train_steps):
        ...
        metrics = adv0.train_on_batch(gen0_inputs,
                                      [y, fake_z0, real_labels]) #real_labels[:,0:num_clusters] #Weil oben bei der erstellung des adv0 auch das array gesliced wurde
...
def create_ts(generator0, generator1, condition):
    ...
    return tss#, condition
    
    
steps = 1500 #original model used 2000 (choose a low number if you)
gen0, gen1, dis0, dis1, losses ,acc_fake,acc_real, adv0, adv1  = build_and_train_models(train_steps=steps)
...
if(False):
    ...
    '''
    condtion_vector_v = np.concatenate([val_label,all_clusters[train_label.shape[0]:train_label.shape[0]+val_label.shape[0],:]], axis = 2)
    condtion_vector_t = np.concatenate([test_label,all_clusters[train_label.shape[0]+val_label.shape[0]:train_label.shape[0]+val_label.shape[0]+test_label.shape[0],:]], axis = 2)

    condition_vector_vt = np.concatenate([condtion_vector_v,condtion_vector_t],axis=0)

    tss, condition = create_ts(gen0,gen1,condition_vector_vt)
    outfile = 'synthetic datasets/' +str(sequence_length) + 'Bearing_fake_data_25_mtss' #add Model Info to title
    np.save(outfile, tss)
    outfile = 'synthetic datasets/' +str(sequence_length) + 'Bearing_fake_data_25_label_mtss' #add Model Info to title
    np.save(outfile, condition)
...
if(True):    
    ...
    for i in range(n_datasets):
        ...
        t_database = create_ts(gen0,gen1,condition_vector)
        t_label = train_label#condition_vector[:,0:1]
    ...
    outfile = 'synthetic datasets/' +str(sequence_length) + 'Bearing_fake_data_10_mtss' #add Model Info to title
    np.save(outfile, syn_database)
    outfile = 'synthetic datasets/' +str(sequence_length) + 'Bearing_fake_data_label_10_mtss' #add Model Info to title
