...
def build_encoder(inputs, cond_shape,feature0_dim): 
    ...
    y = GRU(units =80, return_sequences=True)(x)
    y = GRU(units =40, return_sequences=True)(y)
    y = GRU(units =20 ,return_sequences=True, unroll=True)(y)
    y = Flatten()(y)
    y = Dense(units = 20*seq_length)(y)
    y = Dense(units=int((feature0_dim+20*seq_length)/2))(y)
    y = Dense(units=feature0_dim, activation='relu')(y)
    # Encoder0 or enc0: data to feature0 
    enc0 = Model(inputs=x, outputs=y, name="encoder0")
    ...
    y = Dense(feature0_dim)(feature0)

    y = Dense(120)(y)
    y = Reshape((20,6))(y)
    y = GRU(units =40, return_sequences=True)(y)
    y = GRU(units =20, return_sequences=True)(y)
    y = GRU(units =10 ,return_sequences=False)(y)
    y = Dense(10)(y)
    y = Dense(cond_shape)(y)
...
def train_encoder(model,
    ...
    model.compile(loss='mse',
                  ...
                  metrics=['mse'])
    model.fit(x_train,
              y_train, #enoceder versucht die timeseries sequences dem cluster zuzuordnen. Aber warum? dafür ist doch der cluster al
...
def build_and_train_models(train_steps = 2000):
    ...
    clusters = all_clusters[0:train_label.shape[0],seq_length-1,:]
...
def train(models, data, params):
    ...
    for i in range(train_steps):
        ...
        metrics = adv0.train_on_batch(gen0_inputs,
                                      [y, fake_z0, real_labels]) #Weil oben bei der erstellung des adv0 auch das array gesliced wurde
...
def create_ts(generator0, generator1, condition):
    ...
    condition = np.reshape(condition,[condition.shape[0],condition.shape[1]])
...
if(True):
    ...
    for i in range(n_datasets):
        ...
        t_database = create_ts(gen0,gen1,train_label)
        #t_label = train_label[:,seq_length-1,:]
    ...
    outfile = 'synthetic datasets/' + 'battery_cmapss_400_rows_simple_cond_data' #add Model Info to title
    np.save(outfile, t_database)
    outfile = 'synthetic datasets/' + 'battery_cmapss_400_rows_simple_cond_label' #add Model Info to title
