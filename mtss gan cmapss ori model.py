...
seq_array, label_array,max_value_rul,max_values = dp.preprocessing_cmapps(sl=seq_length,path ='Data/train_FD001.txt',rul_path = 'Data/RUL_FD001.txt', short=True)
Z_dim = 50 #size of latent vector
gen_concat = np.concatenate([seq_array[:,:,3:4],label_array],axis=2)
features_n = seq_array.shape[2]+1 #RUL column will be later added
num_labels = 12 
num_clusters = 12
...
cond_length = seq_length*cond_col+features_n_c #Summe aus RUL, Cycle und Cluster Information
#%% PCA Analysis
...
def generator(inputs,
    ...
    x = ReLU()(x)
    x = UpSampling1D()(x)
    x = Conv1D(features_n, 4, 1, "same")(x) #features_n-2 because rul and cycle column will be concatenated later
...
def build_encoder(inputs, num_labels=features_n, feature0_dim=features_n*seq_length): 
...
def build_generator(latent_codes, feature0_dim=SHAPE[0]*SHAPE[1]):
...
def build_and_train_models(train_steps = 2000):
    ...
    train_n = int((seq_array.shape[0])*.95)
    #X = data[:,:,:-1]
    #y = data[:,-1,-1]
    
    #calculate clusters
    #X = np.concatenate([seq_array,label_array], axis=2) #put the RUL Label into the trainingdata
    X = seq_array
    clusters = cl.birch_clustering(np.concatenate([seq_array,label_array],axis=2),features_n=features_n_c)
    ...
    #Condtion_vector = np.concatenate([cycle,col7,label_array,clusters], axis = 2)
    y = clusters[:,0,0] #get the first element out of a sequence, which is only needed for the label vector
    #label = label_array[:,seq_length-1,:]
    #cycle = seq_array[:,seq_length-1,3:4]
    X = np.concatenate([X,label_array],axis=2)
    label = np.concatenate([cycle,col7,label_array],axis=2) #col7
    del clusters
...
steps = 500 #original model used 2000 (choose a low number if you)
...
#Create fake Sequenz 
if(True):
    seq_array, label_array,max_value_rul,max_values = dp.preprocessing_cmapps(sl=seq_length, short=False)
    X = seq_array
    clusters = cl.birch_clustering(np.concatenate([seq_array,label_array],axis=2),features_n=features_n_c)
    clusters = np.reshape(clusters, [clusters.shape[0],1,1])
    clusters = np.repeat(clusters, seq_length, axis =1) #a cluster is calculated for one sequence, so it must be repeated for the sequence length, to fit the condition vector
    cycle = seq_array[:,:,2:4] #get setting and cycle information of the real Dataset as part of condition vector
    
    col7 = seq_array[:,:,7:8]
    Condtion_vector = np.concatenate([cycle,label_array,clusters], axis = 2)
    y = clusters[:,0,0] #get the first element out of a sequence, which is only needed for the label vector
    ...
    outfile = 'synthetic datasets/' +str(seq_length) + '_cmapps_fake_short_mtss2_2500steps_4_cond_lat50' #add Model Info to title
    np.save(outfile, tss)
